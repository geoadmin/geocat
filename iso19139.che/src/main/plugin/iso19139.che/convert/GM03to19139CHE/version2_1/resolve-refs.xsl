...
<xsl:stylesheet version="1.0"
    ...
    <xsl:template match="int:GM03_2_1Comprehensive.Comprehensive|int:GM03_2_1Core.Core" mode="ResolveRefs">
        <xsl:element name="{local-name(.)}" namespace="{namespace-uri(.)}">
            <xsl:choose>
                <xsl:when test="int:GM03_2_1Core.Core.MD_Metadata/parentIdentifier">
                    <xsl:apply-templates select="int:GM03_2_1Core.Core.MD_Metadata[parentIdentifier]" mode="ResolveRefs">
