...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:gmd="http://www.isotc211.org/2005/gmd"
  ...
  <xsl:template mode="mode-iso19139" priority="300"
       ...
       gts:TM_PeriodDuration|gml:duration|gml320:duration|che:PT_FreeURL]">
    <xsl:param name="schema" select="$schema" required="no"/>
    <xsl:param name="labels" select="$labels" required="no"/>
    ...
    <xsl:variable name="hasPTFreeText"
                  select="count(gmd:PT_FreeText|che:PT_FreeURL) > 0"/>
    <xsl:variable name="hasOnlyPTFreeText"
                  select="count(gmd:PT_FreeText|che:PT_FreeURL) > 0 and count(gco:CharacterString|gmx:Anchor|gmd:URL) = 0"/>
    ...
    <xsl:variable name="theElement"
                  select="if ($isMultilingualElement and $hasOnlyPTFreeText or not($monoLingualValue))
                          then gmd:PT_FreeText|che:PT_FreeURL
    ...
    <xsl:variable name="values">
      <xsl:if test="$isMultilingualElement">
        <xsl:variable name="text"
                      select="normalize-space(gco:CharacterString|gmx:Anchor|gmd:URL)"/>
        ...
        <values>
          ...
          <xsl:for-each select="gmd:PT_FreeText/gmd:textGroup/gmd:LocalisedCharacterString|che:PT_FreeURL/che:URLGroup/che:LocalisedURL">
            ...
            <xsl:variable name="ptFreeElementDoesNotExist"
                          select="count($theElement/parent::node()/
                                        ...
                                        gmd:LocalisedCharacterString[
                                          @locale = concat('#', $currentLanguageId)]|
                                        $theElement/parent::node()/
                                        che:PT_FreeURL/*/
                                        che:LocalisedURL[
                                          @locale = concat('#', $currentLanguageId)]
                                  ) = 0"/>
  ...
  <xsl:template mode="mode-iso19139" priority="99999" match="che:CHE_CI_ResponsibleParty/gn:child[@name = 'individualName']">
    <!-- Do nothing -->
  </xsl:template>
