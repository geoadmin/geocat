...
/** api: method[search]
    ...
    search: function(formOrParams, onSuccess, onFailure, startRecord, updateStore, metadataStore, summaryStore, async){
        ...
        if (typeof formOrParams === 'object') {
            GeoNetwork.util.SearchTools.doQueryFromParams(formOrParams, this, startRecord, suc, fail, updateStore, metadataStore, summaryStore, async);
        } else {
            GeoNetwork.util.SearchTools.doQueryFromForm(formOrParams, this, startRecord, suc, fail, updateStore, metadataStore, summaryStore, async);
        }
    },
    ...
    metadataShow : function(uuid, maximized, width, height) {
        ...
        // Swisstopo show metadata with "classic" metadata view service
    ...
    metadataShowById : function(id, maximized, width, height) {
        ...
        if (this.resultsView) {
            ...
            var win = new GeoNetwork.view.ViewWindow({
                serviceUrl : url,
                currTab : GeoNetwork.defaultViewMode
                        || 'simple',
                catalogue : this,
                maximized : maximized || false,
                record : record,
                metadataUuid : record ? record.get('uuid')
                        : undefined,
                resultsView : this.resultsView
            });
            win.show(this.resultsView);
    ...
    isLoggedIn : function() {
        ...
        if (response.status === 200 && authenticated) {
            ...
            this.showError(OpenLayers.i18n('connectIssue'),
                    OpenLayers.i18n('connectIssueMsg')
                            + this.services.rootUrl + '.');
        } else if (exception) {
            this.checkError();
            return false;
        } else {
    ...
    login : function(username, password) {
        ...
        var loginWindow;
        if (this.casEnabled) {
            loginWindow = window.open(this.URL + '/srv/'
                    + this.LANG + '/login.form?casLogin',
                    '_casLogin',
                    'menubar=no,location=no,toolbar=no', true);
