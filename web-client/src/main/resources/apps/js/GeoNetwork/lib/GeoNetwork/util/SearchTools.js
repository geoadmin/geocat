...
/**
 * api: (define) module = GeoNetwork.util class = SearchTools
 */
/**
 * api: example GeoNetwork.util.SearchTools is used to build queries from a form
 * and send GET request. Results could be processed using
 * OpenLayers.Format.GeoNetworkRecord.
 * 
 ...
 * 
 * GeoNetwork.util.SearchTools.doQueryFromForm(this.formId, cat, 1,
 * this.showResults, null, true); ... showResults: function(response) { var
 * getRecordsFormat = new OpenLayers.Format.GeoNetworkRecord(); var r =
 * getRecordsFormat.read(response.responseText); var values = r.records; ... }
 * 
 */
GeoNetwork.util.SearchTools = {
     ...
     * api:method[doQuery]
     * 
     * :param query: ``String`` the KVP query :param cat:
     * ``GeoNetwork.Catalogue`` Catalogue to query :param recordNum: ``Number``
     * Optional start record number :param onSuccess: ``Function`` Optional
     * function to trigger in case of success :param onFailure: ``Function``
     * Optional function to trigger in case of failure :param updateStore:
     * ``Boolean`` true to update catalogue attached stores. false to not update
     * them. If false, usually a onSuccess function is used to retrieve search
     * results. :param async: ``Boolean`` false to run in synchrone mode.
     * Default is true.
     * 
     * Send a GET query to server url. A query is a KVP string.
     * 
     */
    doQuery : function(query, cat, startRecord, onSuccess, onFailure,
            updateStore, metadataStore, summaryStore, async) {
        OpenLayers.Request
                .GET({
                    url : cat.services.rootUrl + metadataStore.service + "?"
                            + query,
                    ...
                    success : function(result) {
                        ...
                        if (updateStore) {
                            ...
                            var currentRecords = getRecordsFormat
                                    .read(result.responseText);
                            var values = currentRecords.records;

                            // Only update status if the target metadata store
                            // is the catalogue store
                            // Improve, move the status update on the data
                            // change event TODO
                    ...
                    if (values && values.length > 0) {
                            ...
                            if (isCatalogueSStore) {
                                ...
                                // added check for summary.keywords.keyword
                                // otherwise if result has no keywords the
                                // loadData on store fails
                            ...
                            if (cat && isCatalogueMdStore) {
                                cat.updateStatus(currentRecords.from + '-'
                                        + currentRecords.to
     ...
     * 
     * :param formId: ``String`` Form identifier :param cat:
     * ``GeoNetwork.Catalogue`` Catalogue to query :param recordNum: ``Number``
     * Optional start record number :param onSuccess: ``Function`` Optional
     * function to trigger in case of success :param onFailure: ``Function``
     * Optional function to trigger in case of failure :param updateStore:
     ...
     * results.
     * 
     * Build a GET query. A form is composed of one or more fields which are
     * processed by buildQueryFromForm.
     * 
     */
    doQueryFromForm : function(formId, cat, startRecord, onSuccess, onFailure,
            updateStore, metadataStore, summaryStore, async) {
        var query = GeoNetwork.util.SearchTools.buildQueryFromForm(Ext
                .getCmp(formId), startRecord,
                GeoNetwork.util.SearchTools.sortBy, metadataStore.fast);
        GeoNetwork.util.SearchTools.doQuery(query, cat, startRecord, onSuccess,
                onFailure, updateStore, metadataStore, summaryStore, async);
    },
    doQueryFromParams : function(params, cat, startRecord, onSuccess,
            onFailure, updateStore, metadataStore, summaryStore, async) {
        var filters = [], query;
        GeoNetwork.util.SearchTools.addFiltersFromPropertyMap(params, filters,
                startRecord);
     ...
     * Optional start record number :param onSuccess: ``Function`` Optional
     * function to trigger in case of success :param onFailure: ``Function``
     * Optional function to trigger in case of failure
     * 
     * It's assumed the field name follow a convention. It starts with a letter,
     * followed by an underscore. The letters are: * E: no meaning.
     * 
     * In order to define search fuzziness, add a hidden field named
     * E_similarity with the default similarity to be used in Lucene search.
     * 
     */
    buildQueryFromForm : function(form, startRecord, sortBy, fast) {
        var values = GeoNetwork.util.SearchTools.getFormValues(form);
        var filters = [];
        GeoNetwork.util.SearchTools.addFiltersFromPropertyMap(values, filters,
                startRecord);

        return GeoNetwork.util.SearchTools.buildQueryGET(filters, startRecord,
                sortBy, fast);
    },
    /**
     * api:method[populateFormFromParams]
     * 
     * :param formId: ``String`` Form identifier :param map: ``Object`` List of
     * parameters in a map.
     * 
     * Populate form fields based on a list of parameters. Parameters name must
     * be equal to field name.
     * 
     * Search field may not be visible if in a collapsed fieldset (TODO) Test
     * with radio, dates and geometry (TODO)
     */
    populateFormFromParams : function(form, map, createNewField) {
            ...
            if (cur.getName) {
                ...
                if (map[name]) {
                    ...
                    if (cur.getXType() === 'superboxselect') {
                        ...
                        if (cur.mode === 'local') {
                            // Wait the store to load to set the value (eg.
                            // categories, catalogues)
                            cur.getStore().on('load', function() {
                                cur.setValue(value);
                            });
                            // and set the value too if the store is already
                            // loaded (eg. spatial rep.)
                            cur.setValue(value);
                            // The store should be loaded only once, so it
                            // should be fine.
                        } else {
        ...
        if (createNewField) {
            for ( var searchCriteria in map) {
                if (map.hasOwnProperty(searchCriteria)
                        && map[searchCriteria] !== '') {
    ...
    /**
     * private: method[addFiltersFromPropertyMap] Add property in a GeoNetwork
     * filter Check for similarity and set up start and end index of records to
     * returned.
     */
    addFiltersFromPropertyMap : function(values, filters, startRecord) {
        ...
        if (similarity !== undefined) {
            ...
            GeoNetwork.util.SearchTools.addFilter(filters, 'E_similarity',
                    defaultSimilarity);
        }
    ...
    addFilterImpl : function(filters, type, name, value, defaultSimilarity,
        ...
        if (type == 'S') { // starts with
            ...
            if (type.length > 1) {
                ...
                filters.push("similarity" + "="
                        + encodeURIComponent(type.substring(1)));
            }
            filters.push(name + "=" + encodeURIComponent(value));
            if (type.length > 1) {
        ...
        } else if (type == 'T') { // equals with words separated by spaces
            // that must be all present
            Ext.each(value, function(splitted) {
                filters.push(name + "=" + encodeURIComponent(splitted));
            });
    ...
    /**
     * private: method[addFiltersFromPropertyMap] Define default sort order to
     * use for each kind of sort field.
     */
    sortByMappings : {
        relevance : {
            name : 'relevance',
    ...
    /**
     * private: method[buildQueryGET] Build a GET query based on an OGC filter.
     */
    buildQueryGET : function(filter, startRecord, sortBy, fast) {
        ...
        // if (sortBy) {
        // // TODOvar searchInfo =
        // GeoNetwork.util.SearchTools.sortByMappings[sortBy];
    ...
    /**
     * api: method[getFormValues] According to field type, get form values.
     */
    getFormValues : function(form) {
        var result = {};
        form
                .cascade(function(cur) {
                    if (cur.disabled !== true
                            && !(cur.getName && OpenLayers.String.startsWith(
                                    cur.getName(), 'ext-comp'))) { // Check
                        // element
                        // is
                        // enabled
                        // and rendered (ie. visible, eg. field in a collapsed
                        // fieldset)
                        if (cur.isXType('boxselect') || cur.isXType('combo')) {
                            ...
                            /*
                             * a radiogroup is not a container. So cascade
                             * doesn't visit it... don't ask...
                             */
                            ...
                            if (cur.getValue() !== "") {
                                result[cur.getName()] = cur.getValue().format(
                                        'Y-m-d')
