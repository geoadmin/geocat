...
public class FormatReplacer extends AbstractReplacer {
    ...
    @Override
    protected void queryAddExtraClauses(QueryWithCounter query, Element format, String lang) throws JDOMException {
        String name = getFieldValue(format, ".//gmd:name", lang);
        String version = getFieldValue(format, ".//gmd:version", lang);

        addWeightingClause(query, "__name", name);
        addWeightingClause(query, "__version", version);
    }
}
