...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:variable name="wxs-info-loc">
    ...
    <msg id="b" xml:lang="en">is described in online resource section. Run this process to add operations information
    </msg>
    <msg id="a" xml:lang="fr">Le service WMS ou WFS</msg>
    <msg id="b" xml:lang="fr">est décrit dans la section resource en ligne. Exécuter cette action pour ajouter ou
      remplacer les informations relatives aux opérations
    </msg>
  </xsl:variable>
  ...
  <xsl:template name="analyze-add-service-info-from-wxs">
    ...
    <xsl:if test="$srv"><!-- Only apply to service metadata-->
      <xsl:for-each select="$onlineResources">
        <suggestion process="add-service-info-from-wxs" id="{generate-id()}-service" category="onlineSrc"
                    target="srv:containsOperations">
          <name>
