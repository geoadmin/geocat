...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="@xlink:href[starts-with(., 'local://xml.format.get?id=')
      or starts-with(., 'http://www.geocat.ch:80/geonetwork/srv/deu/xml.format.get?')
      or starts-with(., 'http://www.geocat.ch:80/geonetwork/srv/eng/xml.format.get?')
      or starts-with(., 'http://geocat0i.bgdi.admin.ch:80/geonetwork/srv/deu/xml.format.get?')
      or starts-with(., 'http://geocat0i.bgdi.admin.ch:80/geonetwork/srv/eng/xml.format.get?')]"/>    <!-- for MGDI -->
  ...
  <!-- in MGDI, all extents are 'non validated'; keep them -->
  <!--<xsl:template match="*[contains(@xlink:href, 'gn:non_validated')]"/>-->

  <!-- MGDI: remove references to extents 272 and 274 (invalid references) -->
  <xsl:template match="*[starts-with(@xlink:href, 'local://xml.extent.get?id=272&amp;')
      or starts-with(@xlink:href, 'local://xml.extent.get?id=274&amp;')]"/>
  ...
  <xsl:template match="@xlink:href[starts-with(.,
    ...
    <!-- we have to handle duplicate extents (lists are hardcoded based on SQL queries) -->
    <!-- note: we do not take into account the `uuid` value of the subtemplate! -->
    <xsl:variable name="rawId" select="replace(., '.*[^uu]id=([0-9.]+)(&amp;.*|$)', '$1')"/>
    <xsl:variable name="id">
      <xsl:choose>
        <xsl:when test="count($duplicateIds/group[./dupe = $rawId]) > 0">
          <xsl:value-of select="$duplicateIds/group[./dupe = $rawId]/ref"/>
        </xsl:when>
        <xsl:otherwise>
  ...
  -->
  <!-- in MGDI, all keywords are 'non validated'; keep them -->
  ...
  <xsl:template match="@xlink:href[starts-with(., 'local://eng/xml.keyword.get?') or starts-with(., 'local://che.keyword.get?')
      or starts-with(., 'http://www.geocat.ch:80/geonetwork/srv/eng/xml.keyword.get?')]">
                ...
                replace(
                  replace(
                    replace(
                      replace(
                        replace(
                          replace(., '(local://eng/xml.keyword.get\?|local://che.keyword.get\?|http://www.geocat.ch:80/geonetwork/srv/eng/xml.keyword.get\?)(.*)', '$2'),
                          '(&amp;locales=fr,en,de,it|&amp;lang=eng,ger,ita,fre,roh|&amp;locales=en,it,de,fr)', ''
                        ),
                        'local._none_.geocat.ch', 'local.theme.geocat.ch'
                      ),
                      'local._none_.non_validated', 'local.theme.non_validated'
                    ),
                    'local._none_.inspire', 'local.theme.inspire'
                  ),
                  '&amp;multiple=true', ''
                ),
                '&amp;textgroupOnly=true', ''
