...
class Functions extends iso19139.Functions{
    ...
    private static final def CHAR_PATTERN = /\W/
    ...
    def localizedUrlText = { el ->
        ...
        def locStrings = el.'**'.findAll{ it.name() == 'che:LocalisedURL' && !it.text().isEmpty() && !(it.text().length() == 1 && it.text() =~ CHAR_PATTERN)}
        def ptEl = locStrings.find{it.'@locale' == uiCode}
        if (ptEl != null) return ptEl.text()
        def charString = el.'**'.findAll {it.name() == 'gmd:URL' && !it.text().isEmpty() && !(it.text().length() == 1 && it.text() =~ CHAR_PATTERN)}
