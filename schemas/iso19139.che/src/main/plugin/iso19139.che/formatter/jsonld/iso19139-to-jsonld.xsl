...
<xsl:stylesheet
  ...
  <xsl:variable name="requestedLanguageExist"
                select="$lang != ''
                        and count(//che:CHE_MD_Metadata/gmd:locale/*[gmd:languageCode/*/@codeListValue = $lang]/@id) > 0"/>

  <xsl:variable name="requestedLanguage"
                select="if ($requestedLanguageExist)
                        then $lang
                        else //che:CHE_MD_Metadata/gmd:language/*/@codeListValue"/>

  <xsl:variable name="requestedLanguageId"
                select="concat('#', //che:CHE_MD_Metadata/gmd:locale/*[gmd:languageCode/*/@codeListValue = $requestedLanguage]/@id)"/>



  <xsl:template name="getJsonLD"
                mode="getJsonLD" match="che:CHE_MD_Metadata">
	{
