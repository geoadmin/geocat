        ...
        <xsl:element name="{local-name(.)}" namespace="{namespace-uri(.)}">
            ...
            <xsl:if test="@TID">
                ...
                <xsl:if test="ends-with($selfName, 'Comprehensive.Comprehensive.LI_Source')">
                    <xsl:apply-templates mode="ResolveRefs" select="/int:TRANSFER/int:DATASECTION/int:GM03_2Comprehensive.Comprehensive/
                             int:GM03_2Comprehensive.Comprehensive.sourceStepsource[int:source/@REF = $myTID]/int:sourceStep" />
                </xsl:if>
            </xsl:if>
        </xsl:element>
    </xsl:template>
