  ...
  <xsl:template name="getThreddsDateAsUTC">
    ...
    <xsl:choose>
      ...
      <xsl:when test="$sourceDate/@format">
        <xsl:call-template name="getUtcDateTime">
          <xsl:with-param name="dateTime" select="sdf:parse(sdf:new($sourceDate/@format),$sourceDate)"/>
        </xsl:call-template>
      </xsl:when>
      ...
      <xsl:otherwise>
        <xsl:call-template name="getUtcDateTime">
          <xsl:with-param name="dateTime" select="tds-dateformatter:getISODate(tds-dateformatter:new(),$sourceDate)"/>
        </xsl:call-template>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>
  ...
  <xsl:template name="resolveThreddsDateRange">
    ...
    <xsl:variable name="sdfStartDate">
      <xsl:if test="normalize-space($startDate)!=''">
        <xsl:value-of select="format-dateTime($startDate,'[Y0001]-[M01]-[D01] [H01]:[m01]:[s01] [z]')"/>
      </xsl:if>
    </xsl:variable>
    ...
    <xsl:variable name="sdfEndDate">
      <xsl:if test="normalize-space($endDate)!=''">
        <xsl:value-of select="format-dateTime($endDate,'[Y0001]-[M01]-[D01] [H01]:[m01]:[s01] [z]')"/>
      </xsl:if>
    </xsl:variable>
