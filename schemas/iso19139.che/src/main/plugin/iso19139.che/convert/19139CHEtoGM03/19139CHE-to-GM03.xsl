...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template name="lang3_to_lang2">
    ...
    <xsl:choose>
      <xsl:when test="string-length($lang3)=2">
      ...
      <xsl:otherwise>ERROR_<xsl:value-of select="$lang3"/>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>
  ...
  <xsl:variable name="defaultLanguage">
    <xsl:choose>
      <xsl:when
        test="/che:CHE_MD_Metadata/gmd:language/gco:CharacterString|/gmd:MD_Metadata/gmd:language/gco:CharacterString">
        <xsl:for-each
          select="/che:CHE_MD_Metadata/gmd:language/gco:CharacterString|/gmd:MD_Metadata/gmd:language/gco:CharacterString">
