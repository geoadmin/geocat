...
>
  ...
  <xsl:template mode="iso19139"
                match="gmd:contact|gmd:pointOfContact|gmd:distributorContact|gmd:citedResponsibleParty|gmd:userContactInfo|*[@gco:isoType='gmd:CI_ResponsibleParty']"
                priority="5">
    <xsl:param name="schema"/>
    <xsl:param name="edit"/>
              ...
              <tr>
                <td width="50%" valign="top">
                  <table width="100%">
                    ...
                    <xsl:choose>
                      <xsl:when test="$edit = 'true'">
                        <xsl:variable name="link">
                          ...
                          <xsl:value-of
                            select="substring(../@xlink:href, string-length(substring-before(../@xlink:href, 'id'))+4)"/>
                        ...
                        <xsl:choose>
                          <xsl:when test="not(contains(../@xlink:href, 'deleted'))">
                            <a style="cursor:pointer;font-weight: bold;" href="{$link}" target="_userEditTab">Edit
                              Contact
                            </a>
                          </xsl:when>
                          <xsl:otherwise>
                        ...
                        <xsl:apply-templates mode="elementEP"
                                             select="che:individualLastName|geonet:child[string(@name)='individualLastName']">
                          <xsl:with-param name="schema" select="$schema"/>
                          <xsl:with-param name="edit" select="$edit"/>
                        </xsl:apply-templates>
                      </xsl:when>
                      <xsl:otherwise>
                        <xsl:apply-templates mode="simpleElement"
                          ...
                          <xsl:with-param name="title"
                                          select="/root/gui/schemas/iso19139/labels/element[@name='gmd:individualName']/label"/>
                          <xsl:with-param name="text">
                            <xsl:value-of
                              select="(che:individualFirstName|geonet:child[string(@name)='individualFirstName'])/gco:CharacterString"/>
                            <xsl:text> </xsl:text>
                            <xsl:value-of
                              select="(che:individualLastName|geonet:child[string(@name)='individualLastName'])/gco:CharacterString"/>
                          </xsl:with-param>
                        </xsl:apply-templates>
                      </xsl:otherwise>
                    </xsl:choose>
                    ...
                    <xsl:apply-templates mode="elementEP"
                                         select="che:organisationAcronym|geonet:child[string(@name)='organisationAcronym']">
                      <xsl:with-param name="schema" select="$schema"/>
                      <xsl:with-param name="edit" select="$edit"/>
                    </xsl:apply-templates>
                    ...
                    <xsl:when test="gmd:*|che:LocalisedURL">
                      <xsl:variable name="ref">
                        <xsl:choose>
                          ...
                          <xsl:otherwise>
                            <xsl:value-of select="gmd:*/geonet:element/@ref"></xsl:value-of>
                          </xsl:otherwise>
                        </xsl:choose>
                      </xsl:variable>
  ...
  <xsl:template name="PT_FreeURL_Tree">
    ...
    <xsl:variable name="xlinkedAncestor">
    ...
    <xsl:for-each select="$languages">
      ...
      <xsl:variable name="language">
        <xsl:choose>
          ...
          <xsl:otherwise>
            <xsl:value-of select="$code"/> no found in
    ...
    <xsl:choose>
      <xsl:when test="$edit='true' and $isTemplate='true'">
        <select class="md" name="_{geonet:element/@ref}" size="1">
          ...
          <xsl:for-each
            select="/root/gui/isoLang/record[code='eng' or code='fre' or code='ger' or code='ita' or code='roh']">
  ...
  <xsl:template match="che:CHE_MD_Metadata" mode="csv">
    ...
    <metadata>
      ...
      <gmd:individualFirstName>
        <xsl:for-each
          select="gmd:identificationInfo/che:CHE_MD_DataIdentification/gmd:pointOfContact/che:CHE_CI_ResponsibleParty/che:individualFirstName/gco:CharacterString">
          <xsl:value-of select="."/>
          <xsl:if test="position()!=last()">
            <xsl:value-of select="$internalSep"/>
          </xsl:if>
      ...
      <gmd:individualLastName>
        <xsl:for-each
          select="gmd:identificationInfo/che:CHE_MD_DataIdentification/gmd:pointOfContact/che:CHE_CI_ResponsibleParty/che:individualLastName/gco:CharacterString">
          <xsl:value-of select="."/>
          <xsl:if test="position()!=last()">
            <xsl:value-of select="$internalSep"/>
          </xsl:if>
      ...
      <gmd:electronicMailAddress>
        <xsl:for-each
          select="gmd:identificationInfo/che:CHE_MD_DataIdentification/gmd:pointOfContact/che:CHE_CI_ResponsibleParty/gmd:contactInfo/gmd:CI_Contact/gmd:address/che:CHE_CI_Address/gmd:electronicMailAddress/gco:CharacterString">
          <xsl:value-of select="."/>
          <xsl:if test="position()!=last()">
            <xsl:value-of select="$internalSep"/>
          </xsl:if>
      ...
      <che:basicGeodataID>
        <xsl:value-of
          select="gmd:identificationInfo/che:CHE_MD_DataIdentification/che:basicGeodataID/gco:CharacterString"/>
      </che:basicGeodataID>
      >

      <che:basicGeodataIDType>
        <xsl:value-of
          select="gmd:identificationInfo/che:CHE_MD_DataIdentification/che:basicGeodataIDType/che:basicGeodataIDTypeCode/@codeListValue"/>
      </che:basicGeodataIDType>
      >

      <che:dateOfMonitoringState>
        <xsl:for-each
          select="gmd:identificationInfo/che:CHE_MD_DataIdentification/gmd:resourceMaintenance/che:CHE_MD_MaintenanceInformation/che:dateOfMonitoringState/gco:Date">
          <xsl:value-of select="."/>
          <xsl:if test="position()!=last()">
            <xsl:value-of select="$internalSep"/>
          </xsl:if>
      ...
      <xsl:choose>
        <xsl:when
          test="count(gmd:contentInfo/che:CHE_MD_FeatureCatalogueDescription[che:modelType/che:CHE_MD_modelTypeCode/@codeListValue='FeatureDescription'])=1">
          <xsl:variable name="fc"
                        select="gmd:contentInfo/che:CHE_MD_FeatureCatalogueDescription[che:modelType/che:CHE_MD_modelTypeCode/@codeListValue='FeatureDescription']"></xsl:variable>
          ...
          <che:featureCat.FeatureDescription.date>
            <xsl:for-each select="$fc/gmd:featureCatalogueCitation/gmd:CI_Citation//gmd:CI_Date">
              <xsl:value-of select="gmd:date/gco:Date"/><xsl:text> </xsl:text><xsl:value-of
      ...
      <xsl:choose>
        <xsl:when
          test="count(gmd:contentInfo/che:CHE_MD_FeatureCatalogueDescription[che:modelType/che:CHE_MD_modelTypeCode/@codeListValue='UMLdiagram'])=1">
          <xsl:variable name="fc"
                        select="gmd:contentInfo/che:CHE_MD_FeatureCatalogueDescription[che:modelType/che:CHE_MD_modelTypeCode/@codeListValue='UMLdiagram']"></xsl:variable>
      ...
      <xsl:choose>
        <xsl:when
          test="count(gmd:contentInfo/che:CHE_MD_FeatureCatalogueDescription[che:modelType/che:CHE_MD_modelTypeCode/@codeListValue='INTERLIS1'])=1">
          <xsl:variable name="fc"
                        select="gmd:contentInfo/che:CHE_MD_FeatureCatalogueDescription[che:modelType/che:CHE_MD_modelTypeCode/@codeListValue='INTERLIS1']"></xsl:variable>
      ...
      <xsl:choose>
        <xsl:when
          test="count(gmd:contentInfo/che:CHE_MD_FeatureCatalogueDescription[che:modelType/che:CHE_MD_modelTypeCode/@codeListValue='INTERLIS2'])=1">
          <xsl:variable name="fc"
                        select="gmd:contentInfo/che:CHE_MD_FeatureCatalogueDescription[che:modelType/che:CHE_MD_modelTypeCode/@codeListValue='INTERLIS2']"></xsl:variable>
      ...
      <che:portrayalCat.date>
        <xsl:for-each
          select="gmd:portrayalCatalogueInfo/che:CHE_MD_PortrayalCatalogueReference/gmd:portrayalCatalogueCitation/gmd:CI_Citation//gmd:CI_Date">
          <xsl:value-of select="gmd:date/gco:Date"/><xsl:text> </xsl:text><xsl:value-of
          select="gmd:dateType/gmd:CI_DateTypeCode/@codeListValue"/>
          <xsl:if test="position()!=last()">
            <xsl:value-of select="$internalSep"/>
      ...
      <che:portrayalCatalogueURL>
        <xsl:for-each
          select="gmd:portrayalCatalogueInfo/che:CHE_MD_PortrayalCatalogueReference/che:portrayalCatalogueURL/che:PT_FreeURL/che:URLGroup/che:LocalisedURL">
          <xsl:value-of select="."/>
          <xsl:if test="position()!=last()">
            <xsl:value-of select="$internalSep"/>
          </xsl:if>
      ...
      <gmd:maintenanceAndUpdateFrequency>
        <xsl:for-each
          select="gmd:identificationInfo/che:CHE_MD_DataIdentification/gmd:resourceMaintenance/che:CHE_MD_MaintenanceInformation/gmd:maintenanceAndUpdateFrequency/gmd:MD_MaintenanceFrequencyCode/@codeListValue">
          <xsl:value-of select="."/>
          <xsl:if test="position()!=last()">
            <xsl:value-of select="$internalSep"/>
          </xsl:if>
      ...
      <gmd:userDefinedMaintenanceFrequency>
        <xsl:for-each
          select="gmd:identificationInfo/che:CHE_MD_DataIdentification/gmd:resourceMaintenance/che:CHE_MD_MaintenanceInformation/gmd:userDefinedMaintenanceFrequency/gts:TM_PeriodDuration">
          <xsl:value-of select="."/>
          <xsl:if test="position()!=last()">
            <xsl:value-of select="$internalSep"/>
          </xsl:if>
      ...
      <gmd:metadata.maintenanceAndUpdateFrequency>
        <xsl:for-each
          select="gmd:metadataMaintenance/che:CHE_MD_MaintenanceInformation/gmd:maintenanceAndUpdateFrequency/gmd:MD_MaintenanceFrequencyCode/@codeListValue">
          <xsl:value-of select="."/>
          <xsl:if test="position()!=last()">
            <xsl:value-of select="$internalSep"/>
          </xsl:if>
      ...
      <gmd:metadata.userDefinedMaintenanceFrequency>
        <xsl:for-each
          select="gmd:metadataMaintenance/che:CHE_MD_MaintenanceInformation/gmd:userDefinedMaintenanceFrequency/gts:TM_PeriodDuration">
          <xsl:value-of select="."/>
          <xsl:if test="position()!=last()">
            <xsl:value-of select="$internalSep"/>
          </xsl:if>
      ...
      <gmd:onlineSrc>
        <xsl:for-each
          select="gmd:distributionInfo/gmd:MD_Distribution/gmd:transferOptions/gmd:MD_DigitalTransferOptions/gmd:onLine/gmd:CI_OnlineResource/gmd:linkage/che:LocalisedURL">
          <xsl:value-of select="."/>
          <xsl:if test="position()!=last()">
            <xsl:value-of select="$internalSep"/>
          </xsl:if>
      ...
      <srv:service.individualLastName>
        <xsl:for-each
          select="gmd:identificationInfo/che:CHE_SV_ServiceIdentification/gmd:pointOfContact/che:CHE_CI_ResponsibleParty/che:individualLastName/gco:CharacterString">
          <xsl:value-of select="."/>
          <xsl:if test="position()!=last()">
            <xsl:value-of select="$internalSep"/>
          </xsl:if>
      ...
      <srv:service.individualFirstName>
        <xsl:for-each
          select="gmd:identificationInfo/che:CHE_SV_ServiceIdentification/gmd:pointOfContact/che:CHE_CI_ResponsibleParty/che:individualFirstName/gco:CharacterString">
          <xsl:value-of select="."/>
          <xsl:if test="position()!=last()">
            <xsl:value-of select="$internalSep"/>
          </xsl:if>
      ...
      <srv:service.electronicMailAddress>
        <xsl:for-each
          select="gmd:identificationInfo/che:CHE_SV_ServiceIdentification/gmd:pointOfContact/che:CHE_CI_ResponsibleParty/gmd:contactInfo/gmd:CI_Contact/gmd:address/che:CHE_CI_Address/gmd:electronicMailAddress/gco:CharacterString">
          <xsl:value-of select="."/>
          <xsl:if test="position()!=last()">
            <xsl:value-of select="$internalSep"/>
          </xsl:if>
      ...
      <srv:serviceTypeVersion>
        <xsl:for-each
          select="gmd:identificationInfo/che:CHE_SV_ServiceIdentification/srv:serviceTypeVersion/gco:CharacterString">
          <xsl:value-of select="."/>
          <xsl:if test="position()!=last()">
            <xsl:value-of select="$internalSep"/>
          </xsl:if>
  ...
  <xsl:template mode="simpleElement" match="@uom" priority="2">
    ...
    <xsl:param name="text">
      <xsl:choose>
        <xsl:when test="$edit=true()">
          m
          <input class="md" type="hidden" id="_{../geonet:element/@ref}_uom" name="_{../geonet:element/@ref}_uom"
                 value="m" readonly="1"/>
        </xsl:when>
        <xsl:otherwise>
          <xsl:value-of select="."/>
  ...
  <xsl:template name="iso19139.che-brief">
    ...
    <xsl:apply-templates mode="briefster"
                         select="gmd:identificationInfo/gmd:MD_DataIdentification|gmd:identificationInfo/*[@gco:isoType='gmd:MD_DataIdentification']|gmd:identificationInfo/srv:SV_ServiceIdentification|gmd:identificationInfo/*[@gco:isoType='srv:SV_ServiceIdentification']">
    ...
    <xsl:for-each select="gmd:contact/*">
      ...
      <xsl:if test="normalize-space($role)!=''">
        <responsibleParty role="{geonet:getCodeListValue(/root/gui/schemas, 'iso19139', 'gmd:CI_RoleCode', $role)}"
                          appliesTo="metadata">
    ...
    <geonet:info>
      ...
      <category internal="true">
        <xsl:choose>
          <xsl:when
            test="gmd:identificationInfo/srv:SV_ServiceIdentification|gmd:identificationInfo/*[@gco:isoType='srv:SV_ServiceIdentification']">
            service
          </xsl:when>
  ...
  <xsl:template mode="addXMLFragment"
                match="gmd:referenceSystemInfo|geonet:child[@name='referenceSystemInfo' and @prefix='gmd']"
                priority="100"/>
    ...
    <xsl:choose>
      <xsl:when test="$edit">
        ...
        <xsl:variable name="selector">
          <select id="topic{geonet:element/@ref}" class="md topicCategory" name="_{geonet:element/@ref}" size="1"
                  onchange="validateTopicCategory('topic{geonet:element/@ref}')">
            <option name=""/>
  ...
  <xsl:template mode="iso19139GeoBox" match="*">
    ...
    <xsl:variable name="wID">
      <xsl:choose>
        <xsl:when test=".//gmd:westBoundLongitude/gco:Decimal/geonet:element/@ref">
          <xsl:value-of select=".//gmd:westBoundLongitude/gco:Decimal/geonet:element/@ref"/>
        </xsl:when>
    ...
    <xsl:variable name="eID">
      <xsl:choose>
        <xsl:when test="./gmd:eastBoundLongitude/gco:Decimal/geonet:element/@ref">
          <xsl:value-of select="./gmd:eastBoundLongitude/gco:Decimal/geonet:element/@ref"/>
        </xsl:when>
    ...
    <xsl:variable name="nID">
      <xsl:choose>
        <xsl:when test="./gmd:northBoundLatitude/gco:Decimal/geonet:element/@ref">
          <xsl:value-of select="./gmd:northBoundLatitude/gco:Decimal/geonet:element/@ref"/>
        </xsl:when>
    ...
    <xsl:variable name="sID">
      <xsl:choose>
        <xsl:when test="./gmd:southBoundLatitude/gco:Decimal/geonet:element/@ref">
          <xsl:value-of select="./gmd:southBoundLatitude/gco:Decimal/geonet:element/@ref"/>
        </xsl:when>
    ...
    <table style="width:100%">
      <tr>
        <td/>
        <div id="native_{$eltRef}" style="display:none">
      ...
      <tr>
        ...
        <td class="padded" style="width:100%">
          ...
          <xsl:variable name="geom">
            <xsl:value-of
              select="concat('Polygon((', $w, ' ', $s,',',$e,' ',$s,',',$e,' ',$n,',',$w,' ',$n,',',$w,' ',$s, '))')"/>
          </xsl:variable>
          <xsl:call-template name="showMap">
            <xsl:with-param name="edit" select="$edit"/>
            <xsl:with-param name="coords" select="$geom"/>
  ...
  <xsl:template mode="iso19139VertElement" match="*">
    ...
    <xsl:choose>
      ...
      <xsl:when test="util:allowScripting() = false()">
        <div class="md"><xsl:value-of select="substring-before(text(), '.')"/>.<xsl:value-of
          select="substring(substring-after(text(), '.'),0,4)"/>
        </div>
      </xsl:when>
      <xsl:otherwise>
        <input class="md" type="text" id="{$eltRef}"
               value="{substring-before(text(), '.')}.{substring(substring-after(text(), '.'),0,4)}"
               readonly="readonly"/>
        <input class="md" type="hidden" id="_{$eltRef}" name="_{$eltRef}"
               value="{substring-before(text(), '.')}.{substring(substring-after(text(), '.'),0,4)}"
               readonly="readonly"/>
      </xsl:otherwise>
    </xsl:choose>
