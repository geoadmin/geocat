...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template name="metadata-fop-iso19139.che">
    ...
    <xsl:variable name="alternateTitle">
      <xsl:apply-templates mode="elementFop"
                           select="./gmd:identificationInfo/*/gmd:citation/gmd:CI_Citation/gmd:alternateTitle">
        <xsl:with-param name="schema" select="$schema"/>
      </xsl:apply-templates>
    </xsl:variable>
    <xsl:call-template name="blockElementFop">
      <xsl:with-param name="block" select="$alternateTitle"/>
    </xsl:call-template>
    ...
    <xsl:variable name="status">
      <xsl:apply-templates mode="elementFop"
                           select="./gmd:identificationInfo/*/gmd:citation/gmd:CI_Citation/gmd:status">
        <xsl:with-param name="schema" select="$schema"/>
      </xsl:apply-templates>
    </xsl:variable>
    <xsl:call-template name="blockElementFop">
      <xsl:with-param name="block" select="$status"/>
    </xsl:call-template>
    ...
    <xsl:call-template name="blockElementFop">
      ...
      <xsl:with-param name="label">
        <xsl:value-of
          select="/root/gui/schemas/iso19139/labels/element[@name='gmd:onLine']/label"/>
      </xsl:with-param>
    </xsl:call-template>

    <!-- Contact -->
    <xsl:call-template name="responsibleParty">
      <xsl:with-param name="path" select="./gmd:identificationInfo/*/gmd:pointOfContact"/>
    ...
    <xsl:call-template name="blockElementFop">
      ...
      <xsl:with-param name="label">
        <xsl:value-of
          select="/root/gui/schemas/iso19139/labels/element[@name='gmd:keyword']/label"/>
      </xsl:with-param>
    </xsl:call-template>
    ...
    <xsl:call-template name="blockElementFop">
      ...
      <xsl:with-param name="label">
        <xsl:value-of
          select="/root/gui/schemas/iso19139/labels/element[@name='gmd:spatialResolution']/label"
        />
      </xsl:with-param>
    </xsl:call-template>
    ...
    <xsl:if test="./gmd:identificationInfo/*[name(.)!='srv:SV_ServiceIdentification']">
      ...
      <xsl:call-template name="blockElementFop">
        ...
        <xsl:with-param name="label">
          <xsl:value-of
            select="/root/gui/schemas/iso19139/labels/element[@name='gmd:lineage']/label"/>
        </xsl:with-param>
      </xsl:call-template>
    </xsl:if>
    ...
    <xsl:call-template name="blockElementFop">
      ...
      <xsl:with-param name="label">
        <xsl:value-of
          select="/root/gui/schemas/iso19139/labels/element[@name='gmd:resourceConstraints']/label"
        />
      </xsl:with-param>
    </xsl:call-template>
    ...
    <!-- Contact -->
    <xsl:call-template name="responsibleParty">
      <xsl:with-param name="path" select="./gmd:contact"/>
  ...
  <xsl:template name="responsibleParty">
    ...
    <xsl:variable name="poc">
      <xsl:apply-templates mode="elementFop"
                           select="$path/*/che:individualFirstName                                     |
                $path/*/che:individualLastName                                      |
                $path/*/gmd:organisationName                                        |
                $path/*/gmd:positionName                                            |
                $path/*/gmd:contactInfo/*/gmd:address/*/gmd:electronicMailAddress   |
                $path/*/gmd:contactInfo/*/gmd:onlineResource/*/gmd:linkage          |
                $path/*/gmd:contactInfo/*/gmd:address/*/che:streetName              |
                $path/*/gmd:contactInfo/*/gmd:address/*/gmd:postalCode              |
                $path/*/gmd:contactInfo/*/gmd:address/*/gmd:city                    |
                $path/*/gmd:role/gmd:CI_RoleCode/@codeListValue">
        <xsl:with-param name="schema" select="'iso19139'"/>
      </xsl:apply-templates>
    </xsl:variable>
    <xsl:variable name="label" select="/root/gui/schemas/iso19139/labels/element[@name=$name]"/>
    <xsl:call-template name="blockElementFop">
      <xsl:with-param name="block" select="$poc"/>
      <xsl:with-param name="label">
        <xsl:value-of select="$label[1]/label"/>
      </xsl:with-param>
    </xsl:call-template>
  </xsl:template>
