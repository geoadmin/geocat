...
public class ContactReplacer extends AbstractReplacer {
	...
	protected String getElemXPath() {
		return ".//che:CHE_CI_ResponsibleParty/parent::*|.//*[@gco:isoType=\"gmd:CI_ResponsibleParty\"]/parent::*";
	}
	...
	protected int queryAddExtraClauses(StringBuilder query, Element contact, String lang) throws JDOMException {
		...
		String email = getFieldValue(contact, ".//gmd:contactInfo/gmd:CI_Contact/gmd:address/che:CHE_CI_Address/gmd:electronicMailAddress", lang);

		query.append(" +root:\"che:CHE_CI_ResponsibleParty\"");
		query.append(String.format(" +individualLastName:\"%s\"", lastName));
		query.append(String.format(" +individualFirstName:\"%s\"", firstName));
