...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template name="userinfofields">
				<tr>
					...
					<td class="padded"><input class="content" type="text" name="surname" value="{/root/record/surname}"/></td>
				</tr>
				<tr>
					<th class="padded"><xsl:value-of select="/root/gui/strings/firstName"/> (*)</th>
					<td class="padded"><input class="content" type="text" name="name" value="{/root/record/name}"/></td>
				</tr>
				<tr>
					<th class="padded"><xsl:value-of select="/root/gui/strings/address"/></th>
					<td class="padded"><input class="content" type="text" name="address" value="{/root/record/primaryaddress/address}"/></td>
				</tr>
				<tr>
					<th class="padded"><xsl:value-of select="/root/gui/strings/city"/></th>
					<td class="padded"><input class="content" type="text" name="city" value="{/root/record/primaryaddress/city}"/></td>
				</tr>
				<tr>
					<th class="padded"><xsl:value-of select="/root/gui/strings/state"/></th>
					<td class="padded"><input class="content" type="text" name="state" value="{/root/record/primaryaddress/state}" size="8"/></td>
				</tr>
				<tr>
					<th class="padded"><xsl:value-of select="/root/gui/strings/zip"/></th>
					<td class="padded"><input class="content" type="text" name="zip" value="{/root/record/primaryaddress/zip}"/></td>
				</tr>
				<tr>
					<th class="padded"><xsl:value-of select="/root/gui/strings/country"/></th>
					<td class="padded">
						<select class="content" size="1" name="country">
							<xsl:if test="string(/root/record/primaryaddress/country)=''">
								<option value=""/>
							</xsl:if>
							<xsl:for-each select="/root/gui/countries/country">
								<xsl:sort select="."/>
								<option value="{@iso2}">
									<xsl:if test="string(/root/record/primaryaddress/country)=@iso2">
				...
				<tr>
					...
					<td class="padded"><input class="content" type="text" name="email" value="{/root/record/email}"/></td>
				</tr>
				<tr>
					<th class="padded"><xsl:value-of select="/root/gui/strings/organisation"/></th>
					<td class="padded"><input class="content" type="text" name="org" value="{/root/record/organisation}"/></td>
				</tr>
				<tr>
					...
					<td class="padded">
						<select class="content" size="1" name="kind">
							<xsl:for-each select="/root/gui/strings/kindChoice">
								<xsl:sort select="."/>
								<option value="{@value}">
									<xsl:if test="string(/root/record/kind)=@value">
										<xsl:attribute name="selected"/>
									</xsl:if>
									<xsl:value-of select="."/>
								</option>
				...
				<tr>
					...
					<td class="padded">
						<input type="checkbox" id="isadmin" onclick="profileChanged()">
							<xsl:if test="/root/record/profile='Administrator'">
						...
						<input type="hidden" id="user.profile" name="profile" value="{/root/record/profile}"/>
