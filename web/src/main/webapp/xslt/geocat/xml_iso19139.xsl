...
<xsl:stylesheet version="2.0" xmlns:gmd="http://www.isotc211.org/2005/gmd"
  ...
  <xsl:variable name="langId">
    <xsl:choose>
      <xsl:when test="/root/*/gmd:language">
        <xsl:value-of select="concat('#',upper-case(java:twoCharLangCode(/root/*/gmd:language/gco:CharacterString)))"/>
      </xsl:when>
      <xsl:otherwise>
        <xsl:value-of select="concat('#',upper-case(java:twoCharLangCode(/*/gmd:language/gco:CharacterString)))"/>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:variable>
  ...
  <xsl:template match="gmd:MD_Metadata|*[@gco:isoType='gmd:MD_Metadata']" priority="101">
    <gmd:MD_Metadata>
      <xsl:attribute name="xsi:schemaLocation">http://www.isotc211.org/2005/gmd http://www.isotc211.org/2005/gmd/gmd.xsd
        http://www.isotc211.org/2005/gmx http://www.isotc211.org/2005/gmx/gmx.xsd http://www.isotc211.org/2005/srv
