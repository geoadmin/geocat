...
public class XslUtilTest
{
    ...
    {
        assertEquals("25.0", GeocatXslUtil.expandScientific("2.5E1"));
        assertEquals("25", GeocatXslUtil.expandScientific("25"));
        assertEquals("0.25", GeocatXslUtil.expandScientific("2.5E-1"));
        assertEquals("2.5", GeocatXslUtil.expandScientific("2.5E0"));
    }
	...
	public void testTrim() throws Exception {
		...
		String trimmed = GeocatXslUtil.trimPosList(data);
	...
	public void toHyperlinks() throws Exception {
        ...
        String result = GeocatXslUtil.toHyperlinksSplitNodes(data, Configuration.makeConfiguration(null, null)).toString();
        assertEquals(3, result.split("<a").length);
        assertEquals(2, result.split("<p>").length);
        assertEquals(2, result.split("</p>").length);
        assertEquals(4, result.split("<br/>").length);
        assertNotNull(GeocatXslUtil.parse(Configuration.makeConfiguration(null, null), result, true));
	}
