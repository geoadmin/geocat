...
public class UnpublishInvalidMetadataJob implements Schedule, Service {
    ...
    private Record validate(GeonetContext gc, MetadataRecord metadataRecord, Dbms dbms, DataManager dataManager) throws Exception {
        ...
        if (published) {
            ...
            if (!failureRule.isEmpty()) {
                ...
                published = false;
                todayRecord = new Record(metadataRecord.uuid, Validity.fromBoolean(validated), published, entity, failureRule,
                        failureReasons);
    ...
    public static boolean isPublished(String id, Dbms dbms) throws SQLException {
        ...
        List children = dbms.select(
                "SELECT metadataid FROM operationallowed where metadataid=" + id + " and (groupid = 1 or groupid = -1) and operationid " +
                "= 0").getChildren("record");
    ...
    public static class Record {
        ...
        public void insertInto(Dbms dbms) throws SQLException {
            dbms.execute("INSERT INTO publish_tracking (uuid, entity, validated, published, failurerule, failurereasons) VALUES (?,?,?," +
                         "?,?,?)", uuid,
                    entity, String.valueOf(validated.dbCode), codeForDatabase(published), failureRule.replace('"', '\''),
                    failureReasons.replace('"', '\''));
        }
        ...
        public Element toElement() {
            return new Element("record").addContent(new Element("uuid").setText("" + uuid)).addContent(new Element("entity").setText
                    (entity))
